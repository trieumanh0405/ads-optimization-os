...
After the first validated import, the project can refresh the saved source manually from Decision board or automatically every 30 minutes to 6 hours while an authenticated team workspace is open. The recommended default is 60 minutes. Auto-run can create a fresh optimization snapshot immediately after a successful refresh. Server-side interval throttling prevents multiple users or tabs from reading the same Sheet repeatedly within one cycle; a deliberate manual refresh can bypass that throttle. The engine uses the latest source date as its as-of date, reads stored facts in paginated batches, and derives Ad set/Campaign evidence from Ad-level exports when parent rows are not supplied separately.
